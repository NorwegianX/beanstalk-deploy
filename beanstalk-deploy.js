...
function createBeanstalkVersion(
  application,
  bucket,
  s3Key,
  versionLabel,
...
function deployBeanstalkVersion(
  application,
  environmentName,
  versionLabel,
  newEnvironment,
...
function expect(status, result, extraErrorMessage) {
  if (status !== result.statusCode) {
    ...
    if (result.headers["content-type"] !== "application/json") {
      ...
      throw new Error(
        `Status: ${result.statusCode}. Code: ${result.data.Error.Code}, Message: ${result.data.Error.Message}`
      );
    }
  }
}
  ...
  readFile(file)
    ...
    .then(result => {
      if (bucket === null) {
        ...
        bucket =
          result.data.CreateStorageLocationResponse.CreateStorageLocationResult
            .S3Bucket;
      }
    ...
    .then(result => {
      ...
      if (!environmentName) {
        console.log(
          `No environment name given, so exiting now without deploying the new version ${versionLabel} anywhere.`
        );
        process.exit(0);
      }
      deployStart = new Date();
      console.log(
        `Starting deployment of version ${versionLabel} to environment ${environmentName}`
      );
      return deployBeanstalkVersion(
        application,
        environmentName,
        versionLabel,
        newEnvironment,
    ...
    .then(result => {
      ...
      if (waitUntilDeploymentIsFinished) {
        ...
        console.log(
          'Deployment started, parameter "wait_for_deployment" was false, so action is finished.'
        );
        console.log(
          "**** IMPORTANT: Please verify manually that the deployment succeeds!"
        );
        process.exit(0);
      }
    })
    .then(envAfterDeployment => {
      if (envAfterDeployment.Health === "Green") {
        ...
        console.warn(
          `Environment update finished, but environment health is: ${envAfterDeployment.Health}, HealthStatus: ${envAfterDeployment.HealthStatus}`
        );
        process.exit(1);
      }
    })
    ...
    .then(result => {
      if (result.statusCode !== 200) {
        if (result.headers["content-type"] !== "application/json") {
          ...
          throw new Error(
            `Status: ${result.statusCode}. Message: ${result.data}`
          );
          ...
          if (deployVersionConsecutiveThrottlingErrors >= 5) {
            throw new Error(
              `Deployment failed, got ${deployVersionConsecutiveThrottlingErrors} throttling errors in a row while deploying existing version.`
            );
          } else {
            return new Promise((resolve, reject) => {
...
function main() {
  ...
  if (IS_GITHUB_ACTION) {
    ...
    if (
      (process.env.INPUT_WAIT_FOR_DEPLOYMENT || "").toLowerCase() == "false"
    ) {
      waitUntilDeploymentIsFinished = false;
    }

    if (process.env.INPUT_WAIT_FOR_ENVIRONMENT_RECOVERY) {
      waitForRecoverySeconds = parseInt(
        process.env.INPUT_WAIT_FOR_ENVIRONMENT_RECOVERY
      );
    }
    useExistingVersionIfAvailable =
  ...
  console.log(
    "      AWS Access Key: " +
      awsApiRequest.accessKey.length +
      " characters long, starts with " +
      awsApiRequest.accessKey.charAt(0)
  );
  console.log(
    "      AWS Secret Key: " +
      awsApiRequest.secretKey.length +
      " characters long, starts with " +
      awsApiRequest.secretKey.charAt(0)
  );
...
function waitForDeployment(
  ...
  let eventCalls = 0,
    environmentCalls = 0; // Getting throttled on these print out how many we're doing...
      ...
      describeEvents(application, environmentName, start)
        .then(result => {
          ...
          if (wasThrottled(result)) {
            ...
            console.log(
              `Request to DescribeEvents was throttled, that's ${consecutiveThrottleErrors} throttle errors in a row...`
            );
            return;
          }
          ...
          expect(
            200,
            result,
            `Failed in call to describeEvents, have done ${eventCalls} calls to describeEvents, ${environmentCalls} calls to describeEnvironments in ${formatTimespan(
              waitStart
            )}`
          );
          ...
          for (let ev of events) {
            ...
            console.log(
              `${date.toISOString().substr(11, 8)} ${ev.Severity}: ${
                ev.Message
              }`
            );
      ...
      describeEnvironments(application, environmentName)
        .then(result => {
          ...
          if (wasThrottled(result)) {
            consecutiveThrottleErrors++;
            console.log(
              `Request to DescribeEnvironments was throttled, that's ${consecutiveThrottleErrors} throttle errors in a row...`
            );
            if (consecutiveThrottleErrors >= 5) {
              throw new Error(
                `Deployment failed, got ${consecutiveThrottleErrors} throttling errors in a row while waiting for deployment`
              );
            }
          ...
          expect(
            200,
            result,
            `Failed in call to describeEnvironments, have done ${eventCalls} calls to describeEvents, ${environmentCalls} calls to describeEnvironments in ${formatTimespan(
              waitStart
            )}`
          );
          ...
          if (env.VersionLabel === versionLabel && env.Status === "Ready") {
            if (!degraded) {
              ...
              console.log(
                `Status for ${application}-${environmentName} is ${env.Status}, Health: ${env.Health}, HealthStatus: ${env.HealthStatus}`
              );
              ...
              if (env.Health === "Green") {
                ...
                console.warn(
                  `Environment update finished, but health is ${env.Health} and health status is ${env.HealthStatus}. Giving it ${waitForRecoverySeconds} seconds to recover...`
                );
                degraded = true;
                healThreshold = new Date(
                  new Date().getTime() + waitForRecoverySeconds * SECOND
                );
                setTimeout(update, waitPeriod);
              }
            } else {
              if (env.Health === "Green") {
                console.log(
                  `Environment has recovered, health is now ${env.Health}, health status is ${env.HealthStatus}`
                );
                resolve(env);
              } else {
                if (new Date().getTime() > healThreshold.getTime()) {
                  reject(
                    new Error(
                      `Environment still has health ${env.Health} ${waitForRecoverySeconds} seconds after update finished!`
                    )
                  );
                } else {
                  let left = Math.floor(
                    (healThreshold.getTime() - new Date().getTime()) / 1000
                  );
                  console.warn(
                    `Environment still has health: ${env.Health} and health status ${env.HealthStatus}. Waiting ${left} more seconds before failing...`
                  );
                  setTimeout(update, waitPeriod);
                }
              }
            ...
            console.log(
              `${new Date().toISOString().substr(11, 8)} ERROR: ${msg}`
            );
            ...
            if (counter % 6 === 0 && !deploymentFailed) {
              console.log(
                `${new Date()
                  .toISOString()
                  .substr(11, 8)} INFO: Still updating, status is "${
                  env.Status
                }", health is "${env.Health}", health status is "${
                  env.HealthStatus
                }"`
              );
            }
